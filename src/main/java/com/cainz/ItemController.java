...
@Controller("/item-point-ref")
public class ItemController {
    private static final Logger LOG = LoggerFactory.getLogger(ItemController.class);
    private final DynamoRepository dynamoRepository;

    public ItemController(DynamoRepository dynamoRepository) {
    ...
    public String itemPost(@Body String body) throws JSONException {
        ...
        try {
            LOG.info("Input Request Body:" + body);
            JSONObject jsonObject = new JSONObject(body);
            String memberCode = "";
            String pointAll = "";
            String rank = jsonObject.getString("MEMBER_RANK");
            if (jsonObject.has("MEMBER_CODE")) {
                memberCode = jsonObject.getString("MEMBER_CODE");
            }
            if (jsonObject.has("POINT_ALL")) {
                pointAll = jsonObject.getString("POINT_ALL");
            }
            LOG.info("Member code is : {} and point all is :{}", memberCode, pointAll);
            ...
            expressionAttributeValues.put(":janValue", AttributeValue.builder().s(janCodeList.get(0)).build());
            QueryResponse queryResponse = dynamoRepository.queryIndex(expressionAttributesNames ,expressionAttributeValues);
            List<Map<String, AttributeValue>> itemList = queryResponse.items();
            LOG.info("Size of records fetched from database based on jan code : " + itemList.size());
            ...
            LOG.info("Response of the Item Point Reference Method:" + response.toString());
